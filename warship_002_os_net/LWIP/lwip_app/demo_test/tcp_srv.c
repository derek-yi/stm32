...
int main()
{
    ...
    servAddr.sin_port = htons(8087); //端口为3000
    ...
    /* 绑定服务器套接字 */
    if (bind(serverSocket, &servAddr, sizeof(servAddr)) == SOCKET_ERROR) 
    {
    ...
    printf("linstening on port %d...\n", ntohs(servAddr.sin_port));

    /* 阻塞方式等待accept */
    int len = sizeof(cliAddr);
    SOCKET clientSocket;
    clientSocket = accept(serverSocket, &cliAddr, &len);
